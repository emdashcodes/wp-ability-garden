 ...
 * @returns Tuple of [updateFunction, getStreamingId, resetStreamingId]
 */
function createStreamingMessageUpdater(setMessages) {
    ...
    const resetStreamingId = () => {
        streamingMessageId = null;
    };
    return [updateStreamingMessage, getStreamingId, resetStreamingId];
}
export function useWordPressChat(config) {
    const { endpoint, nonce, timeout = 120000, enableStreaming = true, conversationStorageKey, } = config;
    // Use conversation storage hook for messages and processing state
    const { messages, setMessages, isProcessing, setIsProcessing } = useConversationStorage(conversationStorageKey);
    // Additional state
    const [error, setError] = useState(null);
    const [suggestions, setSuggestions] = useState([]);
    const [_messageActionsRegistrations, setMessageActionsRegistrations] = useState([]);
    const [clientAbilities, setClientAbilities] = useState([]);
    // Use extracted hooks
    const { createAbortController, abort: abortRequest, getSignal, } = useAbortController();
    const { makeRequest } = useStreamingRequest();
    const { handleServerToolCall, handleClientToolCall, handleToolCallDelta } = useToolCallHandler(setMessages);
    ...
    const onSubmit = useCallback(async (message) => {
        ...
        try {
            ...
            let assistantMessageWithToolCalls = null;
            let currentCompletionId = null;
            // Create streaming message updater
            const [updateStreamingMessage, getStreamingId, resetStreamingId] = createStreamingMessageUpdater(setMessages);
            ...
            await makeRequest({
                ...
                signal: getSignal(),
                enableStreaming,
            }, payload, updateStreamingMessage, async (chunk) => {
                // Log all chunks
                ...
                if (chunk.id && chunk.id !== currentCompletionId) {
                    ...
                    // Reset the streaming ID so a new message will be created
                    // Keep the old message intact - it contains the pre-tool-call text
                    resetStreamingId();
                    // Signal to clear the accumulator so text doesn't concatenate
                    return { clearAccumulator: true };
                }
                // Capture assistant message with tool_calls
                if (chunk.assistant_message) {
                    assistantMessageWithToolCalls =
                        chunk.assistant_message;
                    debug('[WordPress Chat] Captured assistant message with tool calls');
                }
                ...
                if (chunk.client_tool_call) {
                    ...
                    // Execute client tool
                    const { message: toolResultMessage, skipContinuation, } = await handleClientToolCall(toolCall);
                    // Check if tool wants to skip continuation (e.g., navigate)
                    if (skipContinuation) {
                        debug('[WordPress Chat] Tool requested skip continuation');
                        // Store for post-navigation continuation
                        const storeFunc = window
                            .__wpAbilityToolkit_storeNavigationContinuation;
                        if (typeof storeFunc === 'function') {
                            storeFunc(assistantMessageWithToolCalls, toolResultMessage);
                        }
                        return; // Exit stream - navigation will reload
                    }
                    // Continue conversation with tool result
                    const continuationMessages = assistantMessageWithToolCalls
                        ? [
                            ...wpMessages,
                            assistantMessageWithToolCalls,
                            toolResultMessage,
                    ...
                    debug('[WordPress Chat] Continuing with tool result');
                    // Make continuation request
                    const [updateContinueMessage] = createStreamingMessageUpdater(setMessages);
                    await makeRequest({
                        endpoint,
                        nonce,
                        signal: getSignal(),
    ...
    const continueWithToolResult = useCallback(async (assistantMessage, toolResult) => {
        setIsProcessing(true);
        setError(null);
        const cleanupTimeout = createAbortController(timeout);
        try {
            ...
            // Create streaming message updater
            const [updateStreamingMessage] = createStreamingMessageUpdater(setMessages);
            // Make streaming request
            await makeRequest({
                endpoint,
                nonce,
    ...
    const registerSuggestions = useCallback((newSuggestions) => {
        setSuggestions(newSuggestions);
    }, []);
    const clearSuggestions = useCallback(() => {
        setSuggestions([]);
    }, []);
    const registerMessageActions = useCallback((registration) => {
        setMessageActionsRegistrations((prev) => [...prev, registration]);
    }, []);
    const unregisterMessageActions = useCallback((id) => {
        setMessageActionsRegistrations((prev) => prev.filter((reg) => reg.id !== id));
    }, []);
...
//# sourceMappingURL=useWordPressChat.js.map