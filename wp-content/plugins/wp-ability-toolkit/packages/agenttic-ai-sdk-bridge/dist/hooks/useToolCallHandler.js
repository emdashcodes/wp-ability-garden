 ...
 */
function tryParsePartialJSON(jsonStr, field) {
    try {
...
const ToolCallWrapper = (props) => createElement(ToolCall, {
    toolCall: props.toolCall,
});
export function useToolCallHandler(setMessages) {
    // Track accumulated arguments for streaming tool calls
    const streamingToolArgs = useRef(new Map());
    /**
     * Create a tool call UI message
     */
    const createToolCallMessage = useCallback((toolCallId, toolCallContent) => {
        return {
     ...
     */
    const updateToolCallMessage = useCallback((toolCallId, toolCallContent) => {
     ...
     */
    const handleToolCallDelta = useCallback((delta) => {
        ...
        if (name === 'wp-ability-toolkit/think') {
            ...
            console.log('[THOUGHT]', thought ? `${thought.length} chars` : 'null', 'from', newArgs.length, 'total');
            if (thought !== null) {
                parsedInput = { thought };
            }
            debug(`[Tool Delta] Think - ${thought ? thought.length : 0} chars extracted from ${newArgs.length} total`);
        }
        else {
     ...
     */
    const handleServerToolCall = useCallback((serverToolCall) => {
        const toolCallContent = {
        ...
        if (serverToolCall.status === 'pending') {
            ...
            setMessages((prev) => {
                ...
                if (exists) {
                    ...
                    return prev.map((msg) => msg.id === messageId
                        ? {
                            ...
                            content: [
                                {
                                    type: 'component',
                                    component: (props) => createElement(ToolCall, {
                                        toolCall: props.toolCall,
                                    }),
                                    componentProps: {
                                        toolCall: toolCallContent,
                    ...
                    const message = createToolCallMessage(serverToolCall.id, toolCallContent);
                    return [...prev, message];
                }
            });
        }
     ...
     */
    const handleClientToolCall = useCallback(async (toolCall) => {
        ...
        const skipContinuation = Boolean(toolResult.output &&
...
//# sourceMappingURL=useToolCallHandler.js.map