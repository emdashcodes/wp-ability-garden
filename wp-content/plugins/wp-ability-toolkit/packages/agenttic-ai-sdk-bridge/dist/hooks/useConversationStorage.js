...
export function useConversationStorage(storageKey) {
    // Load initial messages from storage
    const [messages, setMessages] = useState(() => {
        if (storageKey) {
            const savedMessages = loadConversation(storageKey);
            if (savedMessages && savedMessages.length > 0) {
                debug('[Conversation Storage] Restored conversation:', savedMessages.length, 'messages');
    ...
    useEffect(() => {
        if (storageKey && messages.length > 0 && !isProcessing) {
            debug('[Conversation Storage] Saving conversation:', messages.length, 'messages');
    ...
    useEffect(() => {
        if (storageKey && storageKey !== previousStorageKey.current) {
            ...
            const savedMessages = loadConversation(storageKey);
            if (savedMessages && savedMessages.length > 0) {
                debug('[Conversation Storage] Reloaded from new key:', savedMessages.length, 'messages');
...
//# sourceMappingURL=useConversationStorage.js.map