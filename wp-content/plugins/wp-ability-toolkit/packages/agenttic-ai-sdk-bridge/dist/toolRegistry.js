...
export async function getAllClientAbilities() {
    try {
        ...
        const clientAbilities = abilities.filter((ability) => typeof ability.callback === 'function');
        debug('[Tool Registry] Client abilities found:', clientAbilities.length, clientAbilities.map((a) => a.name));
...
export async function executeClientAbility(name, input) {
    try {
        ...
        const abilityInput = input && typeof input === 'object' && !Array.isArray(input)
            ? input
            : {};
        debug(`[Tool Registry] Executing ability ${name} with input:`, abilityInput);
...
export async function executeToolCall(toolCall) {
    try {
        const output = await executeClientAbility(toolCall.name, toolCall.input);
        return {
            id: toolCall.id,
            name: toolCall.name,
            output,
