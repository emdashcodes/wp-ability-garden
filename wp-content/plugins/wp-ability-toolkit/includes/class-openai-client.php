...
class OpenAI_Client extends AI_Client {
	...
	public function stream_chat( $model, $messages, $tools_manager = null, $recursion_depth = 0 ) {
		...
		$this->stream_buffer = '';
		$this->current_chunk_id = '';
		$this->reasoning_started = false;
		...
		// Enable reasoning for o1/o3 models.
		$supports_reasoning = strpos( $model, 'o1' ) !== false || strpos( $model, 'o3' ) !== false;
		if ( $supports_reasoning ) {
			$body_array['reasoning'] = array(
				'effort' => 'medium',
			);
		}
	...
	public function stream_callback( $ch, $data ) {
		...
		foreach ( $lines as $line ) {
			...
			if ( $chunk && isset( $chunk['choices'][0] ) ) {
				...
				// Store chunk ID for thinking block.
				if ( isset( $chunk['id'] ) ) {
					$this->current_chunk_id = $chunk['id'];
				}

				// Handle reasoning content (o1/o3 models).
				if ( isset( $delta['reasoning_content'] ) && ! empty( $delta['reasoning_content'] ) ) {
					// Emit thinking_start on first reasoning content.
					if ( ! $this->reasoning_started ) {
						$this->reasoning_started = true;
						$this->thinking_block_id = 'thinking-' . $this->current_chunk_id;
				...
				if ( isset( $delta['content'] ) && empty( $this->tool_calls ) ) {
					// If we were in reasoning mode, emit thinking_stop before content.
					if ( $this->reasoning_started ) {
						echo 'data: ' . wp_json_encode(
							array(
								'thinking_stop' => array(
