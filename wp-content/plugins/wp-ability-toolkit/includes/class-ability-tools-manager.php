...
class Ability_Tools_Manager {
	...
	/**
	 * Convert abilities to Anthropic Claude tool format
	 *
	 * @return array Anthropic tools array.
	 */
	public function convert_to_anthropic_tools(): array {
		$abilities = $this->get_all_abilities();
		$tools = array();

		foreach ( $abilities as $ability ) {
			// Get input schema or create default.
			$raw_schema = $ability['input_schema'] ?? array();

			// Build a clean Anthropic-compatible input_schema.
			$input_schema = array(
				'type' => 'object',
			);

			// Handle properties - ensure it's an object, not empty array.
			if ( isset( $raw_schema['properties'] ) && is_array( $raw_schema['properties'] ) && ! empty( $raw_schema['properties'] ) ) {
			...
			if ( isset( $raw_schema['required'] ) && is_array( $raw_schema['required'] ) ) {
				$input_schema['required'] = $raw_schema['required'];
			}

			$tools[] = array(
				'name'         => $this->sanitize_tool_name( $ability['name'] ),
				'description'  => $ability['description'] ?? $ability['label'] ?? 'No description',
