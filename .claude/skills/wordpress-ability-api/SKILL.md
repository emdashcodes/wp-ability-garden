...
- **PHP abilities**: Add to plugin's main PHP file or create a dedicated `includes/abilities.php` file and hook into `wp_abilities_api_init`
- **JavaScript abilities**: Add to enqueued JavaScript file with `@wordpress/abilities` dependency
- **Categories**: Register on `wp_abilities_api_categories_init` hook before abilities
...
- Check registration: `wp.abilities.getAbility('namespace/ability-name')`
- Execute ability: `wp.abilities.executeAbility('namespace/ability-name', inputData)`
- Check available abilities: `wp.abilities.getAbilities()`
- Or use the data store: `wp.data.select('abilities').getAbilities()`
